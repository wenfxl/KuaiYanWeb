...
<!--          </el-popover>-->
      ...
      <el-table v-loading="is加载中" :data="Data.List" border style="width: 100% ;white-space: pre-wrap;"
        ...
        <el-table-column prop="uid" label="消费者" width="150" show-overflow-tooltip="">
        ...
        <el-table-column prop="rmb" label="用户价格" width="90"></el-table-column>
        <el-table-column prop="inviterId" label="邀请人" width="150" show-overflow-tooltip="">
          <template #default="scope">
            <el-tag type="success">
              {{ scope.row.inviter }}
            </el-tag>
        ...
        <el-table-column prop="inviterRMB" label="核心佣金" width="120" show-overflow-tooltip="">
          <template #default="scope">
            <el-tag type="info" v-if="scope.row.inviterStatus!=3">
              {{ 支付状态Map[scope.row.inviterStatus] ? 支付状态Map[scope.row.inviterStatus] : "未知状态" }}
            </el-tag>
            ¥{{ scope.row.inviterRMB }}({{ scope.row.inviterDiscount }}%)
          </template>
        </el-table-column>

        <el-table-column prop="grandpaId" label="邀请上级" width="150" show-overflow-tooltip="">
          <template #default="scope">
            <el-tag type="warning">
              {{ scope.row.grandpa?scope.row.grandpa:"无" }}
            </el-tag>
          </template>
        </el-table-column>
        <el-table-column prop="grandpaRMB" label="裂变佣金" width="120" show-overflow-tooltip="">
          <template #default="scope">
            <el-tag type="info" v-if="scope.row.grandpaStatus!=3">
              {{ 支付状态Map[scope.row.grandpaStatus] ? 支付状态Map[scope.row.grandpaStatus] : "未知状态" }}
            </el-tag>
            ¥{{ scope.row.grandpaRMB }}({{ scope.row.grandpaDiscount }}%)
          </template>
        </el-table-column>
        <el-table-column prop="note" label="备注" show-overflow-tooltip="">
          <template #default="scope">
            <el-button link type="primary" size="default"
                       @click="on订单备注被改变(scope.$index,scope.row.id,scope.row.note)" style="color:#79bbff">
...
import { Del批量删除LogRMBPayOrder, SetPayOrderNote} from "@/api/支付充值订单api.js";
import {
  时间_时间戳到时间,
  时间_取现行时间戳,
  时间_计算天时分秒提示,
...
const on批量删除 = async (Type: number) => {
  ...
  is加载中.value = true
  const res = await cps佣金订单api.delete(提交数据)
  is加载中.value = false
  console.log(res)
  if (res.code == 10000) {
...
import {cps佣金订单api} from "@/api/cps佣金订单api";
const { tableRef, tableHeight, updateTableHeight } = useTableHeight(85)
const on表格列宽被改变 = (newWidth: any, oldWidth: any, columns: any, event: any) => {
  let 局_列宽数组: number[] = 表格读取列宽数组(tableRef.value)

  localStorage.setItem('列宽_cps佣金订单', JSON.stringify(局_列宽数组));
}
const on订单备注被改变 = async (表项索引: number, id: number, Note: string) => {
  ...
  await ElMessageBox.prompt('请输入新订单备注', '', {
    ...
    inputValue: Note
      ...
      .then(async ({value}) => {
        新备注 = value
        const res = await cps佣金订单api.setNote ({"ids": [id], "note": 新备注})
        console.log(res)
        if (res.code == 10000) {
...
const on表格列宽初始化 = () => {
  ...
  let 局_列宽数组文本 = localStorage.getItem('列宽_cps佣金订单')
  if (局_列宽数组文本 != null) {
...
const onGetLogRMBPayOrderList = async () => {
  is加载中.value = true
  const res = await cps佣金订单api.getList(对象_搜索条件.value)
  is加载中.value = false
  console.log(res)
  Data.value = res.data
...
onMounted(async () => {
  ...
  onReset()
  if (Store.state.搜索_cps佣金订单.Size != 0 && Store.state.搜索_cps佣金订单.Size != null) {
    对象_搜索条件.value = Store.state.搜索_cps佣金订单
    console.log("恢复搜索条件")
    console.log(Store.state.搜索_cps佣金订单.Size)
    console.log(Store.state.搜索_cps佣金订单)
  }
...
onBeforeUnmount(() => {
  ...
  Store.commit("set搜索_cps佣金订单", 对象_搜索条件.value)
})
