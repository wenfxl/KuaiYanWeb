      ...
      <el-form :inline="false" style="min-width: 80px;min-height: 100%" label-width="160px" :rules="on表单校验"
        ...
        <el-form-item label="运营状态" prop="Status">
          <el-radio-group v-model="data.Status" :size="is移动端()?'small':''">
            <el-popover placement="top" trigger="hover" content="所有用户禁止登录">
              <template #reference>
        ...
        <el-popconfirm
            ...
            width="30%"
            icon-color="#626AEF"
            title="一行一个,格式推荐使用[1.2.5],大版本号.小版本号.编译版本号,第一行为最新版本,自动更新只判断提示大小版本号,主动更新才判断检测编译版本号"
        ...
        <el-popover
            ...
            :width="200"
            trigger="hover"
            :content="isVipType?'只有会员时间未到期才可以读取':'只有剩余点数>0才可以读取'"
        >
          <template #reference>
        ...
        <el-form-item label="通讯加密类型" prop="CryptoType">
          <el-radio-group v-model="data.CryptoType" :size="is移动端()?'small':''">
            ...
            <el-radio-button :label="3">Rsa签名交换Aes密匙</el-radio-button>
          </el-radio-group>
        </el-form-item>

        <el-form-item v-show="data.CryptoType===2" label="加解密密匙" prop="CryptoKey">
          <el-input type="text" v-model="data.CryptoKeyAes"/>
        </el-form-item>

        <el-form-item v-show="data.CryptoType===3" label="Rsa私钥" prop="CryptoKey">
          <el-input type="textarea"  v-model="data.CryptoKeyPrivate"/>
        </el-form-item>

        <el-form-item v-show="data.CryptoType===3" label="Rsa公钥" prop="CryptoKey">
        ...
        <el-form-item v-for="(data,key) in 专属变量" :label="data.Name" :key="key" style="width: 100%">
          <div class="专属变量" style="display: inline-block ;width: 100%">
            <el-input v-if="data.Type===1" type="text" v-model="data.Value" style="width: calc(100% - 70px - 50px)"/>
        ...
        <el-divider>
          <el-button type="primary" size="large" :icon="Plus" style="width: 110px" round @click="is添加专属变量 = true">
            添加专属变量
          </el-button>
        </el-divider>
...
import {CirclePlus, Plus} from "@element-plus/icons";
...
const data = ref({
  ...
  "CryptoType": 1,
  "CryptoKeyAes": "aE0WDmhp3j84STssmc2EypU5HGhoa0H1",
  "CryptoKeyPrivate": "",
  "CryptoKeyPublic": "",
  "MaxOnline": 1,
